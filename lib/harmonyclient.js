...
function getCurrentActivity() {
  ...
  return this.request("getCurrentActivity").then(function(response) {
...
function getActivities() {
  ...
  return this.getAvailableCommands().then(function(availableCommands) {
...
function startActivity(activityId) {
  ...
  return this.request("startactivity", body, "encoded", function(stanza) {
    ...
    if (event && event.attr("type") === "connect.stateDigest?notify") {
      ...
      if (
        activityId === "-1" &&
        digest.activityId === activityId &&
        digest.activityStatus === 0
      ) {
        canHandleStanza = true;
      } else if (
        activityId !== "-1" &&
        digest.activityId === activityId &&
        digest.activityStatus === 2
      ) {
        canHandleStanza = true;
      }
    }
...
function isOff() {
  ...
  return this.getCurrentActivity().then(function(activityId) {
    var off = activityId === "-1";
    debug(
      off
        ? "system is currently off"
        : "system is currently on with activity " + activityId
    );
...
function getAvailableCommands() {
  ...
  return this.request("config", undefined, "json").then(function(response) {
    return response;
  });
}
...
function request(
  ...
  canHandleStanzaPredicate =
    canHandleStanzaPredicate || defaultCanHandleStanzaPredicate.bind(null, id);
